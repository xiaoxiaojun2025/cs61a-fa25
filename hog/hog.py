...
GOAL = 300  # The goal of Hog is to score 100 points.
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    index = 0
    max_value = 0
    rolling_with_i = make_averaged(roll_dice, times_called)
    for i in range(1,11):
        temp = rolling_with_i(i, dice)
        if temp > max_value:
            max_value = temp
            index = i
    return index
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if sus_update(0, score, opponent_score) >= GOAL:
        return 0
    start, end, diff = 1, 10, opponent_score - score
    k = 0.3
    if score <= 10 :
        start = 4
    elif score >= GOAL - 10 :
        ...
        if diff < -10:
            ...
            start = 6
            k -= 0.15
    max_value = sus_update(0, score, opponent_score)
    max_value -= k * boar_brawl(opponent_score, max_value)
    index = 0
    rolling_with_i = make_averaged(sus_update, 20)
    for i in range(start, end + 1):
        temp = rolling_with_i(i, score, opponent_score)
        temp -= k * boar_brawl(opponent_score, temp)
        if temp > max_value:
            max_value = temp
            index = i
    return index
